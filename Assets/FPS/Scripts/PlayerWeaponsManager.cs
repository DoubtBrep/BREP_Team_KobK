...
public class PlayerWeaponsManager : MonoBehaviour
{
    ...
    [Header("Misc")]
    [Tooltip("Which hand is represented by this component")]
    public Hand hand;
    ...
    private void Start()
    {
        ...
        //Persist character accross loads
        //DontDestroyOnLoad(this.transform.gameObject.transform.parent);
    }
    ...
    {
        if (hand == Hand.Right) //we only want to do this for the right hand. 
        {
            if (force || (newWeaponIndex != activeWeaponIndex && newWeaponIndex >= 0))
            {
                ...
                EquipToLeft(m_WeaponSlots[newWeaponIndex]);
            }
        }
    }
    ...
    {
        //print("Get Weapon at slot index returning" + activeWeaponIndex);
    ...
    public int GetIndexOfWeaponFromName(WeaponController weaponPrefab)
    {
        int index = 9;
        for (int i = 0; i < m_WeaponSlots.Length; i++) //hand does have the weapon. find it and return the index value
        {
            if (m_WeaponSlots[i] != null && m_WeaponSlots[i].weaponName == weaponPrefab.weaponName)
            {
                index = i;
            }
        }
    ...
    public void SwitchToWeaponIndexLeft(int newWeaponIndex, bool force = false) //original SwitchToWeaponIndex. Now only used when switching to a left hand dual wield. 
    {
